...
for i in range(0,len(weapons)):
    for j in range(0,len(armor)):
        ...
        if not player_wins(pdamage,parmor,ppoints,bdamage,barmor,bpoints) and pcost > maxcost:
            maxcost = pcost

# 1 ring
maxcost2 = sys.maxsize*-1
for i in range(0,len(weapons)):
    for j in range(0,len(armor)):
        for k in range(0,len(rings)):
            ...
            if not player_wins(pdamage,parmor,ppoints,bdamage,barmor,bpoints) and pcost > maxcost2:
                maxcost2 = pcost

# 2 rings
maxcost3 = sys.maxsize*-1
for i in range(0,len(weapons)):
    for j in range(0,len(armor)):
        for k in range(0,len(rings)):
            for l in range(0,len(rings)):
                ...
                if not player_wins(pdamage,parmor,ppoints,bdamage,barmor,bpoints) and pcost > maxcost3:
                    maxcost3 = pcost

costs = [maxcost,maxcost2,maxcost3]
costs.sort()
print('part 2: ' + str(costs[2])) # 356 (too high)
print()
end_secs = time.time()
