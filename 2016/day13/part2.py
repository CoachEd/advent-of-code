...
def get_steps(y1,x1,y2,x2,distances):
  unvisited = {node: None for node in nodes} #using None as +inf
  visited = {}
  current = str(y1)+','+str(x1)
  currentDistance = 0
...
for y in range(rows):
  for x in range(cols):
    if a[y][x] == 0: # 0 is an empty space (node)
      num_steps = get_steps(1,1,y,x,distances)
      if num_steps > -1 and num_steps <= 50:
        n += 1
