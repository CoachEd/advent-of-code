...
xgrid = [[0 for x in range(c)] for y in range(r)]
...
start_r = port_r
start_c = port_c
wire1_nodes.append([start_r,start_c])
for p in w1:
    r = start_r
    c = start_c
    d = p[0]
    ...
    for r1 in range(start_r,row_bound,row_step):
      for c1 in range(start_c,col_bound,col_step):
        if r1 == start_r and c1 == start_c:
          continue
        wire1_nodes.append([r1,c1])
        if grid[r1][c1] != 'o':
          grid[r1][c1] = '1'
...
start_r = port_r
start_c = port_c
wire2_nodes.append([start_r,start_c])
for p in w2:
    r = start_r
    c = start_c
    d = p[0]
...
# PART 2
temp_dist = 0
for n in wire1_nodes:
  r = n[0]
  c = n[1]
  if r == port_r and c == port_c:
    continue
  temp_dist = temp_dist + 1
  if grid[r][c] == 'X' and xgrid[r][c] == 0:
    # haven't seen this 'X'
    xgrid[r][c] = temp_dist * -1 # negative means update by wire 1

temp_dist = 0
for n in wire2_nodes:
  r = n[0]
  c = n[1]
  if r == port_r and c == port_c:
    continue
  temp_dist = temp_dist + 1
  if grid[r][c] == 'X' and xgrid[r][c] < 0:
    # haven't seen this 'X' a second time
    xgrid[r][c] = abs(xgrid[r][c]) + temp_dist

min = sys.maxsize
for r in range(len(xgrid)):
  for c in range(len(xgrid[r])):
    if xgrid[r][c] != 0:
      if xgrid[r][c] <= min:
        min = xgrid[r][c]
