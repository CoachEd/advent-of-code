...
for y in range(0,len(board)):
  for x in range(0,len(board[y])):
    ...
    lx=x-1
    if board[y][x] == '#':
      # see if it is an intersection
      if validYX(uy,ux,board) and validYX(dy,dx,board) and validYX(ry,rx,board) and validYX(ly,lx,board):
        check_s = board[uy][ux] + board[dy][dx] + board[ry][rx] + board[ly][lx]
        if check_s == '####':
          # found intersection
          alignment_sum = alignment_sum + y*x
    
print('alignment_sum: ' + str(alignment_sum))
