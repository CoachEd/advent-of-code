...
for y in range(rows):
  for x in range(cols):
    if l[y][x] in ['a', 'S']:
      candidates.append(str(y) + ',' + str(x))

dijkstra = Dijkstra(input_vertices, input_graph)

minSteps = sys.maxsize
minNode = ''
for start_vertex in candidates:
  ...
  if steps < minSteps:
    minSteps = steps
    minNode = start_vertex
  
  print("Distance from %s to %s is: %.2f  steps: %s" % (start_vertex, end_vertex, dist,steps))

  #print("Path from %s to %s is: %s" % (start_vertex, end_vertex, " -> ".join(se)))
  #print()

print()
print("closest node %s , steps: %s" % (minNode,minSteps))
